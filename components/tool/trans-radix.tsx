...
export function TransRadixTool() {
  ...
  const [radixSelect1, setRadixSelect1] = useState<number | undefined>(
    undefined,
  )
  const [value1, setValue1] = useState('')
  const [radixRadio2, setRadixRadio2] = useState(10)
  const [radixSelect2, setRadixSelect2] = useState<number | undefined>(
    undefined,
  )
            ...
            >
              {RADIX_OPTIONS.map((option) => (
                <div key={option.value} className="flex items-center gap-1.5">
                  <RadioGroupItem
                    id={`source-radix-${option.value}`}
                    value={option.value}
                  />
                  <Label htmlFor={`source-radix-${option.value}`}>
                    {option.label}
                  </Label>
                </div>
              ))}
          ...
          <div className="space-y-2">
            ...
            >
              {RADIX_OPTIONS.map((option) => (
                <div key={option.value} className="flex items-center gap-1.5">
                  <RadioGroupItem
                    id={`target-radix-${option.value}`}
                    value={option.value}
                  />
                  <Label htmlFor={`target-radix-${option.value}`}>
                    {option.label}
                  </Label>
                </div>
              ))}
