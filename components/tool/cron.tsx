...
${state.isValid ? `下次运行时间:\n${state.nextRuns.map((d) => `  ${d.toLocaleString('zh-CN')}`).join('\n')}` : ''}`
  ...
  const formatDate = (date: Date) => {
    ...
    if (isToday) dayLabel = '今天'
      ...
      dayLabel = date.toLocaleDateString('zh-CN', {
        weekday: 'short',
        month: 'short',
        day: 'numeric',
      })
  ...
  return (
    <div className="mx-auto grid max-w-[1320px] gap-2 xl:grid-cols-[minmax(320px,0.85fr)_minmax(520px,1.15fr)]">
      <div className="space-y-2">
        ...
        <div className="rounded-none border border-slate-200/80 bg-white/92 p-2 shadow-sm dark:border-slate-800 dark:bg-slate-950/78">
          <h3 className="mb-1.5 text-sm font-semibold text-slate-700 dark:text-slate-300">
            Cron 表达式格式
          </h3>
          <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-400">
            <div className="rounded-none bg-slate-100 p-1.5 font-mono dark:bg-slate-800">
              * * * * * *
            </div>
